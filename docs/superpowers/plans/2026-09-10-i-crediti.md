...
## Task 0: Misurare il fornitore — ✅ FATTO il 2026-09-10

**Files:** Create `docs/2026-09-10-misura-nbp.md` ✅

**Esito:** [docs/2026-09-10-misura-nbp.md](../../2026-09-10-misura-nbp.md).
**Il cancello è passato** — 18,1 s di mediana a 1K, 21,2 a 2K, 26,9 il
peggiore, contro un limite di trenta: `/genera` resta **una richiesta sola**.

⚠️ **Ma ha rovesciato quattro cose, e i Task 1, 4 e 7 qui sotto sono già stati
corretti di conseguenza.** Se stai leggendo una versione vecchia di questo
piano: `thinkingLevel` non esiste (400 su ogni chiamata), l'immagine torna in
**JPEG**, `costo_reale` va **calcolato** da `usageMetadata` invece che copiato
dalla stima, e il listino era sotto il vero del 6%.

Gli step qui sotto restano per memoria di come si è misurato.
...
- Produces:
  ...
  - `LISTINO`, `prezzoDi(servizio, { riferimenti = 0 }) => { total, cost, margin }`,
...
async function genera(req, env) {
  ...
  const { servizio, prompt, riferimenti = [], misura = 'rapida' } = await req.json().catch(() => ({}));
  ...
  try {
    const misuraGoogle = voce.misure?.[misura] || voce.misure?.rapida || '1K';
...
- [ ] **Step 5-bis: Ridimensionare i riferimenti PRIMA che partano**

In `src/lib/conto.js`, accanto a `generaImmagine`:
...
e in `Riferimenti.jsx` il conto dei riferimenti alimenta il preventivo, perché
il prezzo sale con loro: `<Preventivo servizio={...} riferimenti={scelti.length} />`.
...
```json
{
  ...
  "rapida": "Rapida",
  "grande": "Grande",
  "attendi": "Zack sta generando. Ci mette una ventina di secondi.",
  "chiudi": "Va bene così",
...
```json
{
  ...
  "rapida": "Quick",
  "grande": "Large",
  "attendi": "Zack is generating. It takes about twenty seconds.",
  "chiudi": "Done",
  "ruolo": { "personaggio": "Character", "oggetto": "Object", "stile": "Style" }
}
```

- [ ] **Step 7-bis: L'attesa si vede**

⚠️ **Diciotto secondi davanti a un tasto sono tantissimi.** Non è un problema
tecnico — il limite è trenta — è che senza niente che si muova l'utente preme
una seconda volta, e la seconda volta **si addebita di nuovo**.

Mentre `generaImmagine` è in volo: il tasto Zack è `disabled`, e sotto compare
`t('immagine.attendi')`, che dice anche **quanto** ci vuole. Un'attesa
annunciata è un'attesa; un'attesa muta è un guasto.
