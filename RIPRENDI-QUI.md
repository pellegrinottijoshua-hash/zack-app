...
| **`overflow` su un elemento flex** | vale per **`auto` quanto per `hidden`**: azzera la dimensione minima automatica. In colonna l'etichetta si restringe a **0 px**, col testo dentro e invisibile. Serve `flex: none`. |
...
| **Una porta a senso unico** | «Correggi a mano» dal Blocco copiava il risultato nel banco a file singolo e **si dimenticava da dove veniva**: all'APPLICA la correzione non aveva più un posto a cui tornare, finiva in libreria come `-corretto` e la piastrella nel Blocco restava sbagliata. Chi apre una cosa altrove deve portarsi dietro la sua identità, o il ritorno non esiste (2026-08-27). |
| **Due pennelli nello stesso prodotto** | `stamp` nel motore e una `pennella` scritta per la home. Due pennelli divergono al primo ritocco di uno solo, e i bordi smettono di combaciare fra studio e home. Collassati in uno il 2026-08-28. |
| **`setPointerCapture` che solleva** | serve a non perdere il tratto se il dito esce dalla tela, ma su un puntatore che il browser non riconosce **solleva** — e senza un `try` attorno interrompe il gesto prima ancora di dipingere. |
| **`behavior: 'smooth'` come parte di un comportamento** | misurato il 2026-08-28: con quell'opzione lo `scrollLeft` restava **0**, senza arrivava al bersaglio esatto. Se una cosa deve **succedere**, non può dipendere dal fatto che un'animazione parta: la morbidezza va nel CSS (`scroll-behavior`), dove è un abbellimento e rispetta da sola chi ha chiesto meno movimento. |
| **`scrollIntoView` su una barra `position: fixed`** | scorre anche la **pagina**, trascinandosi dietro tutto il resto. Si calcola il bersaglio e si muove il contenitore. |
| **Il lato della barriera ricalcolato a ogni timbro** | il righello non serviva a niente **proprio nel gesto per cui esiste**: attraversando la guida il centro del pennello passava dall'altra parte, il lato si ribaltava, e da lì in poi si dipingeva sul lato sbagliato. Il lato lo decide **l'inizio del tratto** (2026-08-28). |
| **Un elemento che deve stare FERMO dentro un contenitore centrato** | non sta fermo. `.lp-primo` era `justify-content: center`, quindi si ridisponeva a ogni cambio di contenuto e la mascotte saltava di 13 px all'arrivo del primo file. Chi deve restare immobile va ancorato **in alto**, e a un contenitore che non cambia misura. |
| **`border-radius: 999px` non è un'ellisse** | arrotonda i lati corti e lascia dritti i lunghi: è uno stadio, non un ovale. Per la stessa forma del tasto Zack serve `border-radius: 50%`, e il prezzo è che dentro un'ellisse gli angoli non si possono usare. |
| **Video → SVG** | non esiste. **Un solo fotogramma vettorializzato pesa più di tutto il video**: 232,7 KB contro i 229 KB dell'mp4 intero, e l'animazione completa 27,5 MB — 123 volte (misurato il 2026-08-27 con `wasm_vtracer` su `zack-1.mp4`). Un codec video codifica la *differenza* fra fotogrammi; un SVG non ha nozione di «prima» e ripaga tutto ogni volta. |
| **Salvare due volte lo stesso risultato** | `saveAsset` non aveva idempotenza: tre `maglietta-jayl-zack` da **8.875.116 byte identici**, cioè il tasto premuto tre volte. Ora l'impronta SHA-256 del contenuto + il nome decidono, e il secondo salvataggio torna indietro con `riusato: true` (2026-08-27). |
| **`defaultModelFor` restituisce il MODELLO, non l'id** | passarlo a `getModel` dà «Modello sconosciuto: [object Object]», che manda a cercare un modello mancante invece di un tipo sbagliato (2026-08-27). |
| **Scaricare il modello al passaggio del mouse** | sembra generoso — si guadagnano i secondi del finder — ma scarica 175 MB anche per un'immagine che si ritaglia in 20 ms senza modello, cioè annulla il motivo per cui «Fondo piatto» esiste. Si scarica **dopo** aver provato l'istantaneo, e solo per i file che ne hanno bisogno. |
| **Guardare la home in un'anteprima headless** | appena la pagina è scorsa dipinge solo il fondo del `body`: schermata nera, e la zona sticky non si compone mai. La home si guarda **in un browser vero** (2026-08-27). **Ma metà del problema era un altro** (2026-08-28): col riquadro nascosto `innerWidth` è **0**, l'intera pagina collassa a zero pixel, e ogni misura presa da lì è finta — un canvas largo 0 fa dividere per zero e il pennello dipinge nel nulla. Prima di misurare qualsiasi cosa: `resize_window` a una misura vera, e controllare `innerWidth`. |
